  ...
  class ClickhouseSpanBackfill
    ...
    def call
      ...
      @scope.in_batches(of: @batch_size) do |batch|
        spans = batch.includes(:project).to_a.reject { |span| span.project.purge_pending? }
        spans.each do |span|
          bucket_keys[[ span.project_id, "span", span.started_at.utc.beginning_of_hour ]] = true
        end
        rows = spans.map do |span|
          SpanIngestor.new(span:, request_context: {}, clickhouse_client: @client).attributes
        end
        existing_keys = existing_span_keys(rows)
        rows.reject! do |row|
          existing_keys.key?([ row.fetch(:project_id).to_i, row.fetch(:span_id).to_s ])
        end
        next if rows.empty?
    ...
    def close_owned_client
      ...
      Rails.logger.warn("clickhouse_span_backfill_close_error error=#{error.class}: #{error.message}")
    end
    ...
    def insert_active_rows(rows)
      ...
      Project.transaction(requires_new: true) do
        ...
        @client.insert_spans!(active_rows)
    ...
    def expanded_bucket_keys(touched_keys)
      ...
      while cursor < ended_at
        project_ids.each { |project_id| keys[[ project_id, "span", cursor ]] = true }
        cursor += 1.hour
      end
      keys.keys
    end
    ...
    def ensure_bucket_limit!(project_ids, started_at, ended_at)
      estimated_bucket_count = project_ids.length * [ ((ended_at - started_at) / 1.hour).ceil, 0 ].max
      return if estimated_bucket_count <= @max_watermark_buckets

      raise ArgumentError,
        "Span watermark reconciliation requires #{estimated_bucket_count} buckets; " \
    ...
    def existing_span_keys(rows)
      return {} if rows.empty?

      values = rows.map do |row|
        span_id = TelemetryIdentity.normalize_uuid(row.fetch(:span_id)) || raise(ArgumentError, "Invalid span UUID")
        ...
        "(#{project_id}, toUUID('#{span_id}'), toUInt128('#{checksum}'), #{projection_version})"
      end.join(", ")
      @client.select_rows!(<<~SQL.squish).to_h do |row|
        SELECT project_id, toString(span_id) AS span_id
        FROM #{@client.spans_table_name}
        WHERE (project_id, span_id, identity_checksum, projection_version) IN (#{values})
      SQL
        [ [ row.fetch("project_id").to_i, row.fetch("span_id").to_s ], true ]
      end
    end
  end
end
