...
module InstanceConfiguration
  class Registry
    ...
    DEFINITIONS = [
      ...
      Definition.new(key: "background_jobs.redis_url", section: "background_jobs", label: "Shared Redis URL", env_key: "REDIS_URL", type: :secret, default: "redis://127.0.0.1:6379/0", secret: true, restart_required: true, help: "Backward-compatible Redis fallback for small installations. Dedicated cache, rate-limit, and Sidekiq URLs take precedence when configured.", placeholder: "rediss://user:password@host:6379/0"),
      Definition.new(key: "background_jobs.cache_redis_url", section: "background_jobs", label: "Cache Redis URL", env_key: "REDIS_CACHE_URL", type: :secret, default: nil, secret: true, restart_required: true, help: "Optional cache-only Redis endpoint. Falls back to REDIS_URL when blank; eviction is acceptable for this endpoint.", placeholder: "rediss://user:password@cache-host:6379/0"),
      Definition.new(key: "background_jobs.rate_limit_redis_url", section: "background_jobs", label: "Rate-limit Redis URL", env_key: "REDIS_RATE_LIMIT_URL", type: :secret, default: nil, secret: true, restart_required: true, help: "Optional rate-limit-only Redis endpoint. Falls back to REDIS_URL when blank.", placeholder: "rediss://user:password@rate-limit-host:6379/0"),
      Definition.new(key: "background_jobs.sidekiq_redis_url", section: "background_jobs", label: "Sidekiq Redis URL", env_key: "REDIS_SIDEKIQ_URL", type: :secret, default: nil, secret: true, restart_required: true, help: "Optional durable Sidekiq endpoint. Falls back to REDIS_URL when blank and must use persistence, high availability, and a noeviction policy.", placeholder: "rediss://user:password@sidekiq-host:6379/0"),
      ...
      Definition.new(key: "clickhouse.circuit_failure_threshold", section: "clickhouse", label: "Circuit failure threshold", env_key: "LOGISTER_CLICKHOUSE_CIRCUIT_FAILURE_THRESHOLD", type: :integer, default: 5, help: "Consecutive transient failures before ClickHouse requests are temporarily stopped.", placeholder: "5"),
      Definition.new(key: "clickhouse.circuit_open_seconds", section: "clickhouse", label: "Circuit open interval", env_key: "LOGISTER_CLICKHOUSE_CIRCUIT_OPEN_SECONDS", type: :integer, default: 30, help: "Seconds to wait before allowing one half-open ClickHouse probe.", placeholder: "30"),
      ...
      Definition.new(key: "authentication.public_api_pre_auth_rate_limit_requests", section: "authentication", label: "Pre-authentication IP attempts", env_key: "LOGISTER_PUBLIC_API_PRE_AUTH_RATE_LIMIT_REQUESTS", type: :integer, default: 12_000, help: "Coarse public API attempt limit per source IP before any credential lookup. Counts accepted and rejected attempts across intake endpoints.", placeholder: "12000"),
