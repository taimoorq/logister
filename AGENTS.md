...
- **Keep docs freshness tied to verified public releases.** `config/ecosystem-versions.json` is the committed catalog of versions already re-read from every public channel. `bin/sync-doc-versions` must use that catalog, never neighboring worktrees or unpublished package metadata. Release-set reconciliation updates the catalog through a reviewed pull request only after all required channels agree.
...
- **Keep `llms.txt` current with the real product shape.** `public/llms.txt` should describe the self-hosted app, supported languages, companion packages, and public docs URLs. It should also point directly to `https://logister.org/docs/llms.txt` as the docs AI map. The docs-specific `cloudflare-docs/llms.txt` should stay focused on public documentation pages. Neither file should drift back to a Ruby-only description.
...
- **Language and platform integrations are first-class project types.** The app currently recognizes Ruby, .NET / ASP.NET Core, Python, JavaScript / TypeScript, CFML, Cloudflare Pages, Android, and iOS projects. Project labels, settings copy, event presenters, and docs links should stay aligned with those supported integration kinds.
- **Client SDKs live in separate repos.** `logister-ruby`, `logister-dotnet`, `logister-python`, `logister-js`, `logister-android`, and `logister-ios` are companion packages with their own release cycles. Do not couple their version numbers or changelogs directly to this Rails app.
- **Mobile SDK package managers are part of the integration contract.** Android publishes as `org.logister:logister-android` through Maven Central after a Sonatype Central Portal deployment is published. iOS publishes through Swift Package Manager from `https://github.com/taimoorq/logister-ios.git` and semantic version tags. Keep [docs/mobile-add-ons.md](docs/mobile-add-ons.md) current when mobile setup or release behavior changes.
...
- **On Fly, database setup belongs in the release phase.** `fly.toml` uses `release_command = './bin/release'`; the web entrypoint should not also prepare the database on every boot.
...
- **A Logister app release starts from `VERSION`.** `VERSION`, the first `CHANGELOG.md` release, and OpenAPI `info.version` must agree. CI never deploys. After CI succeeds for the current protected `main` tip, `release-from-main.yml` creates a new immutable tag only when the version changed; `release.yml` then publishes, verifies, and deploys one canonical container digest. Existing tags enter reconciliation instead of skipping recovery.
...
- **A new Logister app version requires a checked-in release-impact decision.** Update `config/release-impact/*.yml` for every changed contract family and give every consumer an explicit independent bump decision and public reason. Add the target versions and channels to `config/release-sets/v<version>.yml`; `bin/check-release-impact`, `bin/check-release-set`, and the dispatcher must pass before the release is ready.
- **Prepare affected SDK releases in the same sweep.** If a companion package needs code or docs changes to support the app release, update its README, changelog, package version, tests, and release workflow as needed. Bump only the SDKs that changed. Merge the matching version and changelog to protected `main`; the current-main workflow creates the immutable tag only after that commit’s CI succeeds.
- **Keep package managers and GitHub releases aligned.** For SDK repos, a version is not ready if GitHub shows a release but RubyGems, NuGet, npm, PyPI, Maven Central, or Swift Package Manager would not publish the same version from the release workflow. Verify the workflow publishes or exposes the package artifact first, then creates or updates the matching GitHub release, and clearly report any missing secrets, trusted-publisher setup, Sonatype manual publish step, or SwiftPM tag issue.
