...
- **A new Logister app version requires a companion-repo readiness sweep.** When the user asks to create a new version or release of Logister, also inspect `logister-ruby`, `logister-dotnet`, `logister-python`, `logister-js`, `logister-android`, and `logister-ios` before calling the release done. Check whether each SDK already supports any new app behavior, ingestion fields, event shapes, Insights dimensions, docs, or release metadata introduced by the app change.
