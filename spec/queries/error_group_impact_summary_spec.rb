...
RSpec.describe ErrorGroupImpactSummary do
  ...
  it "reports distinct impact, completeness, release range, top cohorts, and a trend" do
    ...
    expect(summary.installations).to have_attributes(value: 2, state: :available, sampled_events: 3, total_events: 3)
    expect(summary.sessions).to have_attributes(value: 2, state: :available, sampled_events: 2, total_events: 3)
